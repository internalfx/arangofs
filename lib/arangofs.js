...
let arangoFS = function (arangoConfig, options = {}) {
  ...
  arango.useDatabase(arangoConfig.database)

  if (arangoConfig.username && arangoConfig.password) {
    arango.useBasicAuth(arangoConfig.username, arangoConfig.password)
  }
  ...
  let destroyFileById = async function (_id) {
    let cursor = await arango.query(aql`
    ...
    cursor = await arango.query(aql`
      FOR file IN ${fileCollection}
        FILTER file.sha256 == ${file.sha256} AND file.status != "Deleted"
        COLLECT WITH COUNT INTO length
        return length
    `)
    let duplicateCount = await cursor.next()
